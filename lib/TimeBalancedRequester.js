    ...
    setInterval(function() {
        ...
        for (index in that.responders) {
            for (id in that.responders[index]) {
                if (now.getTime() - that.responders[index][id].sent > that.CALCULATION_TIMEOUT) {
...
TimeBalancedRequester.prototype.send = function() {
    ...
    if (this.sock && this.sock.socks) {
        ...
        if (len > 0 && _.size(this.responders) === len) { //have data for all sockets
            for (var idx in this.responders) {
                var item = this.responders[idx];
                ...
                this.responders[idx].average = average;
            }
    ...
    var cb = function(err, res) {
        ...
        if (lastRequests && lastRequests[id]) {
            lastRequests[id].time = (new Date().getTime() - lastRequests[id].sent); //save response time
        }
